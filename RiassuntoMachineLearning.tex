...
\begin{figure}[H] 
	...
	Se sono correlati grazie alla parte dopo la sommatoria, possiamo eliminare gli attributi ridondanti.
	...
	\end{figure}
	In generale possiamo dove possiamo usare uno di questi algoritmi per il calcolo della distanza, possiamo anche usare la classificazione per gestire istanze sconosciute.
...
\item Ci sono delle possibili ottimizzazioni che mi fanno usare una struttura di ricerca ad albero per approssimare in modo efficiente il funzionamento di K-nn.
		\end{itemize}Per ogni punto che voglio classificare devo calcolare la distanza da tutti i putni che stanno nel piano, quindi è un costo O(N) dove N è il numero di punti nel piano.
...
Possono modellare le relazioni tra i dati senza fare assunzioni a priori
Possono gestire dati eterogenei
Implementano in modo automatico la feature selection
Sono robusti quando nei dati ci sono errori e outlier
	...
	\begin{figure}[H] 
		...
		\begin{itemize}
			...
			\begin{figure}[H] 
				...
				\subsection{Inferenza con i decision tree}
...
\begin{figure}[H] 
	...
	È importante notare che la somma di tutte queste probabilità sarà sempre uguale a 1.
		...
		\subsection{Creare il decision Tree ottimo}
...
Questa matrice viene moltiplicata per la transformation matrix F che ha dimensione $DxK$, si tratta di una matrice che p ortonormale
...
i primo K autovalori e non considerare tutti gli altri.
...
\begin{itemize}
\item Supponiamo di avere una matrice simmetrica $A$, questa che stiamo considerando non è la matrice con i nostri dati perchè non sarbebe 
